  ...
  return <div>
    <div className="process_inserter_container pl-5 pt-5">
      ...
      <div className="arrival_burst_time_container">
        <div>
          ...
          <Input type="number" placeholder="AT" className="max-w-24" min={0} value={arrivalTime == -1 ? '' : arrivalTime} onChange={(e)=>setArrivalTime(Number(e.target.value))}/>
        </div>
        <div>
          <label htmlFor="burst time" className="font-semibold">Burst Time</label>
          <Input type="number" placeholder="BT" className="max-w-24" min={0} value={burstTime == -1 ? '' : burstTime} onChange={(e)=>setBurstTime(Number(e.target.value))} />
        </div>
      </div>
      ...
      <div className="priority_container">
        { algorithm === 'priority_preemptive' || algorithm === 'priority_non_preemptive' ?
          ...
          transition={{
            ...
            <Input type="number" placeholder="Priority" className="max-w-24 " min={0} value={priority == -1 ? '' : priority} onChange={(e)=>setPriority(Number(e.target.value))} />
          </motion.div>
          : ''
        }
      </div>
    ...
    { algorithm === 'round_robin' ?
      ...
      className="pl-5 pt-5">
        ...
        <Input type="number" placeholder="TQ" className="max-w-28" value={timeQuantum} min={0} onChange={(e)=>setTimeQuantum(Number(e.target.value))}/>
      </motion.div>
      : ''
    }
  </div>
